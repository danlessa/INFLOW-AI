...
def get_future_dates(data):
    ...
    future_dates = cleaning_utils.get_dates_of_interest(last_date, end_date)[
        1:7]
...
def check_if_new_data(data_path='data/temporal_data_seasonal_df.csv'):
    ...
    missing_dates = cleaning_utils.get_dates_of_interest(last_date, today_date)[
        1:]
...
def create_dataframe():
    ...
    teleconnections = pd.read_csv(
        'data/historic/teleconnections.csv', index_col='date')
    inundation_temporal_scaled = pd.read_csv(
        'data/historic/inundation_temporal_scaled.csv', index_col='date')
    gridded_rainfall_temporal = pd.read_csv(
        'data/historic/gridded_rainfall_temporal.csv', index_col='date')
    gridded_rainfall_cumulative_temporal = pd.read_csv(
        'data/historic/gridded_rainfall_cumulative_temporal.csv', index_col='date')
    gridded_moisture_temporal = pd.read_csv(
        'data/historic/gridded_moisture_temporal.csv', index_col='date')

    # Calculate inundation delta
    inundation_temporal_unscaled = pd.read_csv(
        'data/historic/inundation_temporal_unscaled.csv', index_col='date')
    inundation_temporal_delta = inundation_temporal_unscaled[[
        'percent_inundation']].diff()
    ...
    temporal_data_df = pd.concat([
        ...
        rainfall,
        teleconnections,
        inundation_temporal_scaled.rename(
            {'percent_inundation': 'inundation_temporal'}, axis=1)[['inundation_temporal']],
        gridded_rainfall_temporal.rename({'rainfall': 'rainfall_3d_temporal'}, axis=1)[
            ['rainfall_3d_temporal']],
        gridded_rainfall_cumulative_temporal.rename(
            {'cumulative_rainfall': 'rainfall_cumulative'}, axis=1)[['rainfall_cumulative']],
        gridded_moisture_temporal.rename({'moisture': 'moisture_3d_temporal'}, axis=1)[
            ['moisture_3d_temporal']],
        inundation_temporal_delta
    ], axis=1)
    temporal_data_df = cleaning_utils.impute_missing_values(temporal_data_df, temporal_data_df.drop(
        columns=['inundation_temporal', 'inundation_delta']).columns)

    # Create month-day index and load saved seasonal statistics for scaling
    temporal_data_df['month_day'] = pd.to_datetime(
        temporal_data_df.index).strftime('%m-%d')
    ...
    # Exclude the month_day column
    for column in temporal_data_seasonal_df.columns[:-1]:
        ...
        for index, row in temporal_data_seasonal_df.iterrows():
            ...
            if month_day in means.index:
                ...
                # If no data for that month_day, keep original value
                normalized_value = row[column]
    ...
    temporal_data_seasonal_df = temporal_data_seasonal_df.drop(
        'month_day', axis=1).dropna()
...
def custom_loss(y_true, y_pred):
    ...
    sign_penalty = tf.reduce_mean(
        tf.where(tf.sign(y_true) != tf.sign(y_pred), 20.0, 1.0))

    # Penalty for sum mismatch
    # Sum of each sequence (across sequence steps)
    sum_true = tf.reduce_sum(y_true, axis=1)
    sum_pred = tf.reduce_sum(y_pred, axis=1)  # Sum of each predicted sequence
    # Penalise if sums are different
...
def predict_new_inundation_transformer(data, model_path='model/temporal_model.keras', custom_loss_function=custom_loss):
    ...
    model_delta = load_model(model_path, custom_objects={
...
def predict_new_inundation_rf(data, model_path='model/temporal_model.pkl', pca_path='model/pca_model.pkl'):
    ...
    tree_preds = np.array([tree.predict(X_pred_flat)
                          for tree in model_delta.estimators_])
...
def re_scale_predictions(data, y_pred, X_pred, future_dates, model_delta, monte_carlo=True, lower_bounds=None, upper_bounds=None):
    ...
    inundation_temporal_unscaled = pd.read_csv(
        'data/historic/inundation_temporal_unscaled.csv', index_col='date').reindex(data.index)
    inundation_temporal_unscaled = cleaning_utils.impute_missing_values(
        inundation_temporal_unscaled, inundation_temporal_unscaled.columns)
    ...
    if monte_carlo:
        ...
        preds_mean, preds_std = monte_carlo_predictions(
            model_delta, X_pred, num_samples=num_samples)
        preds_mean_unscaled = y_pred[0] * \
            index_stds.values + index_means.values
    ...
    lower_bounds_unscaled[0] = lower_bounds[0] * \
        index_stds.values + index_means.values
    upper_bounds_unscaled[0] = upper_bounds[0] * \
        index_stds.values + index_means.values

    # Loop through predictions to calculate total inundation from deltas
    inundation_pred[0] = prior_inundation + y_pred_unscaled[0][0]
    lower_bound_unscaled_inundation[0] = prior_inundation + \
        lower_bounds_unscaled[0][0]
    upper_bound_unscaled_inundation[0] = prior_inundation + \
        upper_bounds_unscaled[0][0]
    for j in range(1, 6):
        inundation_pred[0][j] = inundation_pred[0][j - 1] + \
            y_pred_unscaled[0][j]
        lower_bound_unscaled_inundation[0][j] = lower_bound_unscaled_inundation[0][j -
                                                                                   1] + lower_bounds_unscaled[0][j]
        upper_bound_unscaled_inundation[0][j] = upper_bound_unscaled_inundation[0][j -
                                                                                   1] + upper_bounds_unscaled[0][j]
...
def print_trigger(inundation_pred, future_dates):
    ...
    inundation_unscaled = pd.read_csv(
        'data/historic/inundation_temporal_unscaled.csv', index_col='date')[['percent_inundation']]

    # Export S-EAP trigger if activated
    pred_max = np.max(inundation_pred[:, :], axis=1)
    season_min = inundation_unscaled['percent_inundation'].rolling(
        window=18, min_periods=1).min().iloc[-1]
...
def export_csv(inundation_pred, lower_bound_unscaled_inundation, upper_bound_unscaled_inundation, future_dates):
    ...
    predictions.to_csv(
        f'{folder_path}/{future_dates[0]}_to_{future_dates[-1]}.csv')
...
def export_graphs(data, future_dates, inundation_pred, lower_bound_unscaled_inundation,
    ...
    # Ensure pred_dates are in datetime format
    pred_dates = pd.to_datetime(future_dates)
    pred_dates = np.insert(pred_dates, 0, data.index[-1])

    # Format prediction data
    inundation_pred_formatted = np.insert(
        inundation_pred[0], 0, inundation_temporal_unscaled['percent_inundation'].iloc[-1])
    lower_bound_unscaled_inundation_formatted = np.insert(
        lower_bound_unscaled_inundation[0], 0, inundation_temporal_unscaled['percent_inundation'].iloc[-1])
    upper_bound_unscaled_inundation_formatted = np.insert(
        upper_bound_unscaled_inundation[0], 0, inundation_temporal_unscaled['percent_inundation'].iloc[-1])
    ...
    for i in range(len(x_lims)):
        ...
        plt.plot(pd.to_datetime(data.index), inundation_temporal_unscaled['percent_inundation'],
                 label='Flood Coverage per MODIS Satellite Data', marker='o', linestyle='-', color='blue')
        plt.plot(pred_dates, inundation_pred_formatted,
                 label='Predicted Flood Coverage (Updated November 1st)', marker='x', linestyle='--', color='red')
        ...
        # Set ticks to appear each year
        plt.gca().xaxis.set_major_locator(mdates.YearLocator())

        plt.xlabel('Year')
        plt.ylabel('Flood Coverage Over INFLOW Study Area (%)')
        plt.ylim(inundation_temporal_unscaled['percent_inundation'].min(
        ) - 0.01, inundation_temporal_unscaled['percent_inundation'].max() + 0.01)
        plt.title(
            f"Predicted Flood Coverage Over INFLOW Study Area, {pred_dates[1].date().strftime('%Y-%m-%d')} to {pred_dates.max().date().strftime('%Y-%m-%d')}")
        plt.xlim(pd.Timestamp(x_lims[i]), pd.Timestamp(
            future_dates[-1]) + timedelta(days=100))
        plt.legend()
        plt.grid(True)
        ...
        plt.savefig(
            f"{folder_path}/prediction_{pred_dates[1].date().strftime('%Y-%m-%d')}_to_{pred_dates.max().date().strftime('%Y-%m-%d')}_{x_lim_names[i]}.png", dpi=300)
        plt.close()

    # Convert the index to a datetime index if it's not already
    inundation_df = pd.DataFrame(
        inundation_temporal_unscaled, columns=['percent_inundation'])
    ...
    predicted_inundation_df = pd.DataFrame(
        inundation_pred_formatted, columns=['percent_inundation'])
    ...
    # Colors from Tableau color palette
    ...
    for i, (year, group) in enumerate(combined_inundation.groupby('Year')):
        if year == datetime.today().year:
            ...
            ax.plot(group['DayOfYear'][:-len(final_predictions)], group['percent_inundation']
                    [:-len(final_predictions)], color='red', label=year, linewidth=2)

            # Plot the last 7 points as dashed
            ax.plot(final_predictions['DayOfYear'], final_predictions['percent_inundation'],
                    color='red', linestyle='--', linewidth=2)

            # Fill the area between the lower and upper bounds for the last 7 predictions
            # Use the date index for the last 7 points
            last_seven_dates = final_predictions.index
            ax.fill_between(final_predictions['DayOfYear'],
                            # Lower bound of last 7 predictions
                            lower_bound_unscaled_inundation_formatted[-len(
                                final_predictions):],
                            # Upper bound of last 7 predictions
                            upper_bound_unscaled_inundation_formatted[-len(
                                final_predictions):],
                            color='red', alpha=0.2, label='95% Confidence Interval')
        else:
            # Assign different shades of blue and green for other years
            # Normalize index for colormap
            color = cmap(i / len(combined_inundation['Year'].unique()))
            ax.plot(group['DayOfYear'], group['percent_inundation'],
                    color=color, label=year, alpha=0.6)
    ...
    # Formatting the plot
    ax.set_title(
        f"Flood Coverage Over INFLOW Study Area, {pred_dates[1].date().strftime('%Y-%m-%d')} to {pred_dates.max().date().strftime('%Y-%m-%d')} ({datetime.today().year} Highlighted in Red)")
    ax.set_xlabel('Month of Year')
    ax.set_ylabel('Flood Coverage Over INFLOW Study Area (%)')
    # Optional: Major ticks by month
    ...
    folder_path = f"predictions/inundation_predictions_{pred_dates[1].date().strftime('%Y-%m-%d')}_to_{pred_dates.max().date().strftime('%Y-%m-%d')}"
    plt.savefig(
        f"{folder_path}/prediction_{pred_dates[1].date().strftime('%Y-%m-%d')}_to_{pred_dates.max().date().strftime('%Y-%m-%d')}_year_by_year_comparison.png", dpi=300)
    plt.close()
