...
def import_mjo():
    # Import mjo dataset
    df_mjo = pd.read_table(os.path.join(
        FOLDER_PATH, "mjo.txt"), delim_whitespace=True, skiprows=1)
    return df_mjo
...
def clean_mjo(df_mjo):
    ...
    # Remove future values (missing)
    df_mjo = df_mjo[df_mjo['mjo20E'] != '*****']
    ...
    for column_name in df_mjo.columns:
        ...
        if 'mjo' in column_name:
            ...
            df_mjo[column_name] = pd.to_numeric(
                df_mjo[column_name], errors='coerce')
