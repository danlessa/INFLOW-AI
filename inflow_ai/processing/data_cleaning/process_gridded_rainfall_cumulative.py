...
def read_stats(region='all'):
    ...
    """
    rainfall_cumulative_mean = gridded_data_stats.gridded_rainfall_cumulative_stats[
        region]['mean']
    rainfall_cumulative_std = gridded_data_stats.gridded_rainfall_cumulative_stats[
        region]['std']
...
def get_new_dates(rainfall_data_path='data/historic/gridded_rainfall_temporal.csv',
    ...
    try:
        gridded_rainfall_temporal = pd.read_csv(
            rainfall_data_path, index_col=0)
        gridded_rainfall_cumulative_temporal = pd.read_csv(
            cumulative_rainfall_data_path, index_col=0)
        new_dates = gridded_rainfall_temporal.iloc[len(
            gridded_rainfall_cumulative_temporal):].index.tolist()
        return new_dates
    except FileNotFoundError:
        logging.error(f"File not found: {data_path}")
        return []
...
def load_new_gridded_rainfall_data(temporal_data_path='data/historic/gridded_rainfall_temporal.csv',
    ...
    with h5py.File(cum_data_path, 'r') as gridded_rainfall_cumulative:
        ...
        # Load the new rainfall grids
        gridded_rainfall_cumulative_last = np.expand_dims(
            dataset[-1, :, :], axis=0)
...
def update_gridded_rainfall_cumulative(
    ...
    try:
        ...
        if gridded_rainfall_new is None:
            ...
            rainfall_3d_array_cumulative_last_unstandardised = unstandardize_array(
                gridded_rainfall_cumulative_last, cum_rainfall_mean, cum_rainfall_std)

            # Cumulative sum most recent values
            new_cumsum = np.cumsum(np.concatenate(
                (rainfall_3d_array_cumulative_last_unstandardised, gridded_rainfall_new), axis=0), axis=0)[1:]

            # Standard scale new data based on saved values
            new_data = standardize_array(
                new_cumsum, cum_rainfall_mean, cum_rainfall_std)
            ...
            rainfall_cumulative_temporal = pd.DataFrame(
                {'cumulative_rainfall': new_data.sum(axis=(1, 2))})
            rainfall_cumulative_temporal['date'] = new_dates
            temporal_mean, temporal_std = read_stats(region='all_temporal')
            rainfall_cumulative_temporal['cumulative_rainfall'] = (
                rainfall_cumulative_temporal['cumulative_rainfall'] - temporal_mean) / temporal_std
            ...
            for i in range(len(regions_gdf)):
                ...
                temporal_mean_region, temporal_std_region = read_stats(
                    region=region_code)
                rainfall_cumulative_temporal[f"cumulative_rainfall_{region_code}"] = np.nansum(
                    region_area, axis=(1, 2)) / (total_cells - np.sum(np.isnan(region_area[0])))
                rainfall_cumulative_temporal[f"cumulative_rainfall_{region_code}"] = (
                    rainfall_cumulative_temporal[f"cumulative_rainfall_{region_code}"] - temporal_mean_region) / temporal_std_region
            ...
            with h5py.File(data_path, 'a') as hdf:
                ...
                logging.info(
                    f"Updated cumulative rainfall dataset shape: {dset.shape}")

            # Update temporal data
            rainfall_cumulative_temporal_historic = pd.read_csv(
                temporal_data_path)[:old_dataset_length]  # Crop to length of spatial data
            rainfall_cumulative_temporal_new = pd.concat(
                [rainfall_cumulative_temporal_historic, rainfall_cumulative_temporal])

            # Save the updated temporal data
            rainfall_cumulative_temporal_new.to_csv(
