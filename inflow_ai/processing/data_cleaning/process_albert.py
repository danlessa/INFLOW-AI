...
client: py_hydroweb.Client = py_hydroweb.Client(
    "https://hydroweb.next.theia-land.fr/api", api_key="70s88Dpwc0UraJi9n4EouE7UXCbXzQJkSdCeT1cLNy4EcczioA")
    ...
    with zipfile.ZipFile(downloaded_zip_path, 'r') as zip_ref:
        ...
        if target_file in files_in_zip:
            extracted_path = zip_ref.extract(
                target_file, path="data/downloads/lake_levels/temp_extraction")
...
def load_and_preprocess_data(file_path, skiprows=47):
    ...
    """
    columns = ["decimal_year", "measurement_date", "time",
               "height", "std", "area", "volume", "mission"]
    albert = pd.read_csv(file_path, skiprows=skiprows,
                         sep=';', names=columns, index_col=False)

    # Convert date and rename columns
    albert['measurement_date'] = pd.to_datetime(
        albert['measurement_date']).dt.strftime('%Y-%m-%d')
...
def align_with_dates(albert, dates_list):
    ...
    albert['date'] = albert['measurement_date'].apply(
...
def update_albert():
    ...
    try:
        ...
        albert = cleaning_utils.impute_missing_values(
            albert, ['albert_water_level'])
