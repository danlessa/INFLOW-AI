...
def date_df(timestep, startdate, enddate, endmonth):
    ...
    df["Pentad"] = "0"
    df.loc[df.Day.apply(lambda x: x in ["01", "02", "03",
                        "04", "05"]), "Pentad"] = "1"
    df.loc[df.Day.apply(lambda x: x in ["06", "07", "08",
                        "09", "10"]), "Pentad"] = "2"
    df.loc[df.Day.apply(lambda x: x in ["11", "12", "13",
                        "14", "15"]), "Pentad"] = "3"
    df.loc[df.Day.apply(lambda x: x in ["16", "17", "18",
                        "19", "20"]), "Pentad"] = "4"
    df.loc[df.Day.apply(lambda x: x in ["21", "22", "23",
                        "24", "25"]), "Pentad"] = "5"
    df.loc[
        df.Day.apply(lambda x: x in ["26", "27",
                     "28", "29", "30", "31"]), "Pentad"
    ] = "6"
    df["Dekad"] = "0"
    df.loc[
        df.Day.apply(
            lambda x: x in ["01", "02", "03", "04",
                            "05", "06", "07", "08", "09", "10"]
        ),
        "Dekad",
    ] = "1"
    ...
    if timestep == "daily":
        ...
        df["StartDate"] = df.Date.apply(lambda x: x.replace(day=1))
        df["EndDate"] = df.Date + \
            pd.to_timedelta(df.PentadLength - 1, unit="D")
        ...
        df["StartDate"] = df.Date.apply(lambda x: x.replace(day=1))
        df["EndDate"] = df.Date + \
            pd.to_timedelta(df.SeasonLength - 1, unit="D")
        length = []
...
def rfe_fname_constructor(timestep, url, version, date, degrade_tag, resolution):
    ...
    fullpath = url + '/' + 'v' + version + '/' + timestep + \
        '/' + str(yyyy) + '/' + str(mm) + '/' + fname_str

    if degrade_tag == 1:
        fname_str = fname_str.split(f'.v{version}')[
            0] + '_' + str(resolution) + f'.v{version}' + fname_str.split(f'.v{version}')[1]
        fullpath = url + '/' + 'v' + version + '/' + timestep + '/' + \
            str(resolution) + '/' + str(yyyy) + '/' + str(mm) + '/' + fname_str

    return fullpath


def sm_fname_constructor(timestep, url, version, date):
    """Deduce SM filename.
    ...
    mm = date[4:6]
    if timestep == 'daily':
        fname_str = f'sm{yyyy}_{mm}_{date[6:8]}.v{version}.nc'
    elif timestep == 'pentadal':
        fname_str = f'sm{yyyy}_{mm}-pt{date[6]}.v{version}.nc'
    elif timestep == 'pentadal-anomalies':
        fname_str = f'sm{yyyy}_{mm}-pt{date[6]}_anom.v{version}.nc'
    elif timestep == 'dekadal':
        fname_str = f'sm{yyyy}_{mm}-dk{date[6]}.v{version}.nc'
    elif timestep == 'dekadal-anomalies':
        fname_str = f'sm{yyyy}_{mm}-dk{date[6]}_anom.v{version}.nc'
    elif timestep == 'monthly':
        fname_str = f'sm{yyyy}_{mm}.v{version}.nc'
    elif timestep == 'monthly-anomalies':
        fname_str = f'sm{yyyy}_{mm}_anom.v{version}.nc'
    elif timestep == 'seasonal':
        fname_str = f'sm{yyyy}_{mm}_seas.v{version}.nc'
    elif timestep == 'seasonal-anomalies':
        fname_str = f'sm{yyyy}_{mm}_seas_anom.v{version}.nc'
...
def get_filenames(remoteurl, daterange, timestep, version, resolution=None, product='rfe'):
    ...
    if product == 'sm':
        dataurl = remoteurl + 'soil_moisture/data'
    elif degrade_tag == 0:
        dataurl = remoteurl + 'rfe/data'
    else:
        dataurl = remoteurl + 'rfe/data_degraded'

    files_to_download = []
    if timestep == 'daily':
        daily_dates = date_df('daily', dt.strftime(
            daterange[0], '%Y-%m-%d'), dt.strftime(daterange[-1], '%Y-%m-%d'), endmonth=False)
        daily_dates = daily_dates.Year.astype(
            'str') + daily_dates.Month.astype('str') + daily_dates.Day.astype(str)
        if product == 'rfe':
            files_to_download = [rfe_fname_constructor(timestep, dataurl, str(
                version), x, degrade_tag, str(resolution)) for x in list(daily_dates)]
        elif product == 'sm':
            files_to_download = [sm_fname_constructor(
                timestep, dataurl, str(version), x) for x in list(daily_dates)]
    elif timestep == 'pentadal':
        pentad_dates = date_df('pentadal', dt.strftime(
            daterange[0], '%Y-%m-%d'), dt.strftime(daterange[-1], '%Y-%m-%d'), endmonth=False)
        pentad_dates = pentad_dates.Year.astype(
            'str') + pentad_dates.Month.astype('str') + pentad_dates.Pentad.astype(str)
        if product == 'rfe':
            files_to_download = [rfe_fname_constructor(timestep, dataurl, str(
                version), x, degrade_tag, str(resolution)) for x in list(pentad_dates)]
        elif product == 'sm':
            files_to_download = [sm_fname_constructor(
                timestep, dataurl, str(version), x) for x in list(pentad_dates)]
    elif timestep == 'dekadal':
        dekad_dates = date_df('dekadal', dt.strftime(
            daterange[0], '%Y-%m-%d'), dt.strftime(daterange[-1], '%Y-%m-%d'), endmonth=False)
        dekad_dates = dekad_dates.Year.astype(
            'str') + dekad_dates.Month.astype('str') + dekad_dates.Dekad.astype(str)
        if product == 'rfe':
            files_to_download = [rfe_fname_constructor(timestep, dataurl, str(
                version), x, degrade_tag, str(resolution)) for x in list(dekad_dates)]
        elif product == 'sm':
            files_to_download = [sm_fname_constructor(
                timestep, dataurl, str(version), x) for x in list(dekad_dates)]
    elif timestep == 'monthly':
        month_dates = date_df('monthly', dt.strftime(
            daterange[0], '%Y-%m-%d'), dt.strftime(daterange[-1], '%Y-%m-%d'), endmonth=False)
        month_dates = month_dates.Year.astype(
            'str') + month_dates.Month.astype('str')
        if product == 'rfe':
            files_to_download = [rfe_fname_constructor(timestep, dataurl, str(
                version), x, degrade_tag, str(resolution)) for x in list(month_dates)]
        elif product == 'sm':
            files_to_download = [sm_fname_constructor(
                timestep, dataurl, str(version), x) for x in list(month_dates)]

    return (files_to_download)


def check_input_values(timestep, resolution, product='rfe'):
    ...
    """
    valid_products = ['rfe', 'sm']
    if product not in valid_products:
        product_check = False
        print("'product' not recognised - excepted values are 'rfe' and 'sm'")
    else:
        product_check = True
    ...
    if all(x for x in [product_check, timestep_check, resolution_check]):
        return True
    else:
        return False
...
def download(request):
    ...
    if check_input_values(timestep, resolution, product):

        # Determine date range given supplied start/end dates.
        daterange = determine_daterange(startdate, enddate)

        # Get list of files to download
        files_to_download = get_filenames(
            remoteurl, daterange, timestep, version, resolution, product)
...
def extract(request):
    ...
    if extract_type in ['point', 'area_average', 'domain']:
        if extract_type == 'point':
            ...
            print('Extracting point TAMSAT estimates for longitude: %s and latitude: %s' % (
                lon, lat))
            ...
            print('Extracting %s TAMSAT rainfall estimates for N: %s, S: %s, W: %s and E: %s' % (
                extract_type, str(N), str(S), str(W), str(E)))

        product = request['product']
        if 'product' not in request:
            product = 'rfe'
        ...
        # List expected files
        daterange = determine_daterange(startdate, enddate)
        flist_expect = get_filenames(
            localdata_dir + '/tamsat/', daterange, timestep, version, resolution, product)
        ...
        if len(flist_exist) > 0:
            if len(flist_exist) != len(flist_expect):
                print('Warning! Not all files within date range found: %s expected, %s found.' % (
                    len(flist_expect), len(flist_exist)))
        ...
        if len(flist_exist) > 0:
            ...
            for file in flist_exist:
                ...
                if extract_type == 'point':
                    ...
                    if str(resolution) == '0.0375':
                        ds_list.append(
                            ds.sel(lon=slice(W, E), lat=slice(N, S)))
                    else:
                        ds_list.append(
                            ds.sel(lon=slice(W, E), lat=slice(S, N)))
            ...
            if len(ds_list) > 0:
                if extract_type == 'point':
                    ...
                    ds = xr.concat(ds_list, dim='time').mean(
                ...
                if extract_type == 'point':
                    ...
                    fname = 'TAMSATv' + str(version) + '_' + timestep + '_' + str(resolution) + '_' + str(
                        lon) + '_' + str(lat) + '_' + startdate + '_' + enddate + '.csv'
                    ...
                    df = ds.to_dataframe().round(4)
                    fname = 'TAMSATv' + str(version) + '_' + timestep + '_' + str(resolution) + '_' + str(
                        N) + '_' + str(S) + '_' + str(W) + '_' + str(E) + '_' + startdate + '_' + enddate + '.csv'
                    fname_full = localdata_dir + '/extracted_data/' + extract_type + '/' + fname
                    if not os.path.exists(os.path.dirname(fname_full)):
                        os.makedirs(os.path.dirname(fname_full))
                    ...
                    fname = 'TAMSATv' + str(version) + '_' + timestep + '_' + str(resolution) + '_' + str(
                        N) + '_' + str(S) + '_' + str(W) + '_' + str(E) + '_' + startdate + '_' + enddate + '.nc'
                    fname_full = localdata_dir + '/extracted_data/' + extract_type + '/' + fname
                    if not os.path.exists(os.path.dirname(fname_full)):
                        os.makedirs(os.path.dirname(fname_full))
            ...
            print(
                'No files found, please check input parameters or that TAMSAT data exists for given parameters.')
...
# TAMSAT data URL
remoteurl = 'https://gws-access.jasmin.ac.uk/public/tamsat/'
